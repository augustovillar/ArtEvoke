...
- `instance_type`: EC2 instance type
  - **GPU instances** (recommended for ML/AI workloads): 
    - `g4ad.xlarge` (8GB VRAM) - **Recommended for your 8GB requirement**
    - `g4dn.xlarge` (16GB VRAM), `g4dn.2xlarge` (16GB VRAM), `g5.xlarge` (24GB VRAM)
...
**GPU Instances (recommended for ML/AI):**
- **EC2 g4dn.xlarge**: ~$0.526/hour (~$380/month) - 16GB VRAM
- **EC2 g4dn.2xlarge**: ~$0.752/hour (~$540/month) - 16GB VRAM, more CPU/RAM
- **EC2 g5.xlarge**: ~$1.006/hour (~$725/month) - 24GB VRAM

## Security Considerations

1. **SSH Access**: Consider restricting SSH to your IP only in the security group
2. **Firewall**: Use `ufw` on the VM to add an extra layer of security
3. **SSL/TLS**: Set up Let's Encrypt certificates for HTTPS
4. **Updates**: Regularly update the system: `sudo apt update && sudo apt upgrade`
5. **Backups**: Set up automated backups for your data volumes

## Cleanup

**Warning**: This will delete the VM and all data on it!
