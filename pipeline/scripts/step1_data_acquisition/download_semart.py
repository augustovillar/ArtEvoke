...
# Adjust paths for new pipeline structure
RAW_DATA_DIR = os.path.join(SCRIPT_DIR, "..", "..", "data", "raw", "SemArt")
URL_SEMART = "https://researchdata.aston.ac.uk/id/eprint/380/1/SemArt.zip"
ZIP_FILE = os.path.join(RAW_DATA_DIR, "SemArt.zip")
MERGED_CSV = "semart_original.csv"
KEEP_FOLDER = "Images"
SEM_ART_DIR = os.path.join(RAW_DATA_DIR, "SemArt")
IMG_DIR_OLD = os.path.join(SEM_ART_DIR, "Images")
IMG_DIR_NEW = os.path.join(RAW_DATA_DIR, "Images")
EXPORT_DIR = os.path.join(RAW_DATA_DIR, "semart_info")
...
def clean_directory():
    for item in os.listdir(RAW_DATA_DIR):
        ...
        if item not in ["SemArt", os.path.basename(IMG_DIR_NEW), os.path.basename(EXPORT_DIR)]:
...
if __name__ == "__main__":
    print(f"Working directory: {RAW_DATA_DIR}")
