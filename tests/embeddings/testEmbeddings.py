...
def load_your_descriptions():
    ...
    df = df.rename(columns={"IMAGE_FILE":"image_file","DESCRIPTION": "original_description", "description_"+model_name: "generated_description"})
...
def load_embedding_model(name):
    ...
    if name in ["ViT-G/14", "ViT-H-14-378-quickgelu (DFN)", "ConvNeXt-XXLarge", "ConvNeXt-Large_320px"]:
        ...
        class OpenCLIPWrapper:
            ...
            def encode_images(self, image_paths, normalize_embeddings=True, batch_size=120):
                all_embeddings = []
                with torch.no_grad():
                    for i in range(0, len(image_paths), batch_size):
                        batch_paths = image_paths[i:i+batch_size]
                        imgs = [self.transform(Image.open(p).convert("RGB")).unsqueeze(0) for p in batch_paths]
...
def generate_embeddings(df, model, generate_faiss):	
    ...
    df["IMAGE_FILE_FULLPATH"] = IMAGE_DIR + "/" + df["image_file"]#.str.replace("test_LMMs/SemArt_sample2000/", "", regex=False)
    ...
    print(f"Time taken to generate generated embeddings: {generated_emb_time:.2f} seconds")
...
def create_faiss(model, generated_embs, image_embs, df, name, image_emb_together):
    ...
    if image_emb_together and hasattr(model, "encode_images"):
        ...
        faiss.write_index(index, os.path.join(FAISS_DIR, f"index/combined_embeddings_{name}.index"))

        with open(os.path.join(FAISS_DIR, f"metadata/combined_embeddings_{name}.pkl"), "wb") as f:
            pickle.dump(combined_metadata, f)

        print(f"FAISS index created and saved for {name} with images and text together.")
        ...
        faiss.write_index(index_text, os.path.join(FAISS_DIR, f"index/text_embeddings_{name}.index"))

        with open(os.path.join(FAISS_DIR, f"metadata/text_embeddings_{name}.pkl"), "wb") as f:
        ...
        if hasattr(model, "encode_images"):
            ...
            faiss.write_index(index_img, os.path.join(FAISS_DIR, f"index/image_embeddings_{name}.index"))

            with open(os.path.join(FAISS_DIR, f"metadata/image_embeddings_{name}.pkl"), "wb") as f:
...
def compute_recall(I, k_values, metadata, df, verbose=False):
    ...
    for k in k_values:
        print(f"Recall@{k}: Text = {tag_counter[k]['text']}, Image = {tag_counter[k]['img']}")

    return recalls

def open_faiss(type, name):
    index = faiss.read_index(os.path.join(FAISS_DIR, f"index/{type}_embeddings_{name}.index"))
    with open(os.path.join(FAISS_DIR, f"metadata/{type}_embeddings_{name}.pkl"), "rb") as f:
...
def calculate_recall(model, name, original_emb, df, image_emb_together, force_text_only=False):
    ...
    for i in indices:
        ...
        for rank in range(len(combined_I[i])):
            ...
            print(f"  Rank {rank+1}: {retrieved_image} [{tag}] (Score: {combined_D[i][rank]:.4f}) {mark}")
...
if __name__ == "__main__":
    ...
    for name, _ in model_names.items():
        ...
        original_emb, generated_embs, image_emb = generate_embeddings(df, model, generate_faiss)
